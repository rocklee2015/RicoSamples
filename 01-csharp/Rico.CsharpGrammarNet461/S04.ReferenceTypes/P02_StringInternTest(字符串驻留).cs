...
namespace Rico.S03.ReferenceType
{
    ...
    {
        /*
         *  IsInterned ：字符串位于驻留池时，则返回对字符串的引用(如果是字符串常量则返回驻留池中的引用，
         *                若是动态生成字符串则返回其新地址引用,因为哈希表不维护动态生成字符串)，否则将返回null引用；
         */
        ...
        [TestMethod]
        public  void IsInterned_Return_StringSelf()
        {
            string str = "abc";

            var actual = string.IsInterned(str);

            Assert.AreEqual("abc", actual);
        }

        [TestMethod]
        public  void IsInterned_Contact_A_StringValue_Return_Null()
        {
            string str = "ab";
            str += "c";
        ...
        [TestMethod]
        public void IsInterned_Contact_A_StringValue_Return_abc()
        {
            string strA = "abc";
            ...
            var actual1 = string.IsInterned(strA);

            Assert.AreEqual("abc", actual1);


            var actual2 = string.IsInterned(strB);

            Assert.AreEqual("abc", actual2);

            //字符串A 和 字符串B 不是同一个引用
            Assert.IsFalse(ReferenceEquals(strA, strB));
        }
