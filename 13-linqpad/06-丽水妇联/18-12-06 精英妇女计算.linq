<Query Kind="SQL">
  <Connection>
    <ID>528e09b9-d416-4159-8623-8cca7676ec17</ID>
    <Persist>true</Persist>
    <Server>192.168.1.218</Server>
    <SqlSecurity>true</SqlSecurity>
    <UserName>sa</UserName>
    <Password></Password>
    <Database>Wfsp.Test</Database>
    <ShowServer>true</ShowServer>
  </Connection>
</Query>

select a.id, a.name,a.birthday,PermanentAddress as Address,RealAddress,BirthAddress,a.lan ,a.lng,c.display as honor  from women a 
inner join honor b on a.id =b.womenid
inner join basedatavalue c on b.honorid=c.id
where c.isdeleted=0 and a.isdeleted=0 and a.PermanentAddress like '%丽水%'